...
var DB = (function () {
  ...
  /* ── SESSION ──
     Sesi disimpan di localStorage (bukan sessionStorage)
     sehingga tidak hilang saat browser di-refresh.         */
  function getSession()     { return read(K.session); }
  function saveSession(u)   { write(K.session, u); }
  function clearSession()   { remove(K.session); }

  /* Ambil data user terbaru dari DB berdasarkan session.
     Ini pastikan data selalu sinkron meski ada perubahan profil. */
  function getCurrentUser() {
    var sess = getSession();
    if (!sess) return null;
    var fresh = findUser(sess.username);
    if (fresh) {
      saveSession(fresh); // update session dengan data terbaru
      return fresh;
    }
    return sess; // fallback ke session data
  }
...
var UI = (function () {
  ...
  function updateNav(user) {
    ...
    if (user) {
      ...
      badgeEl.style.display  = user.isAdmin ? 'inline-block' : 'none';
      uploadEl.style.display = user.isAdmin ? 'inline-block' : 'none';
  ...
  function resetUploadForm() {
    ...
    _scriptItemCount = 0;
    addScriptItem(); // selalu mulai dengan 1 item
  }
  ...
  function addScriptItem() {
    ...
    div.innerHTML =
      '<div class="script-item-header">' +
        '<span class="script-item-num">Script #' + n + '</span>' +
        '<button class="script-item-del" onclick="document.getElementById(\'' + id + '\').remove()" title="Hapus">✕</button>' +
      '</div>' +
      '<div class="form-group" style="margin-bottom:10px">' +
        '<label class="form-label">Nama Script</label>' +
        '<input class="form-input" type="text" placeholder="Contoh: Auto Farm v2" data-sname />' +
      '</div>' +
      '<div class="form-group" style="margin-bottom:0">' +
        '<label class="fl form-label">Kode Script</label>' +
        '<textarea class="form-input form-textarea" rows="5" placeholder="-- Paste kode di sini..." data-scode></textarea>' +
      '</div>';
...
var Scripts = (function () {
  ...
  function upload() {
    ...
    for (var i = 0; i < items.length; i++) {
      var name = items[i].querySelector('[data-sname]').value.trim();
      var code = items[i].querySelector('[data-scode]').value.trim();
      if (!name || !code) {
        UI.showAlert('errUpload', 'Isi nama dan kode untuk setiap script!'); return;
      }
  ...
  function _buildCommentEl(c, scriptId) {
    ...
    if (me) {
      ...
      replyBtn.addEventListener('click', function () {
        toggleReplyInput(c.id, scriptId);
      });
    }
